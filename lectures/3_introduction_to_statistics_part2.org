...
* A (mathematical) probabilistic model                             :noexport:
** 
*** Probabilities
\vspace{-.3em}
...
** Evaluating and Comparing Alternatives With Confidence Intervals :noexport:
...
#+BEGIN_EXPORT latex
  \begin{center}
    \begin{overlayarea}{.9\linewidth}{4.5cm}
      \begin{center}%
        \includegraphics<1>[scale=.911]{fig/2sample_comp_1.fig}% ,subfig=1
        \includegraphics<2>[scale=.911]{fig/2sample_comp_2.fig}% ,subfig=2
        \includegraphics<3->[scale=.911]{fig/2sample_comp_3.fig}% ,subfig=3
      \end{center}
    \end{overlayarea}
  \end{center}
  \vspace{-.8em}
...
- C.I.s overlap because variance is large. Some *setups* may have an
  ...
  $\Var(\rv{A})$ and $\Var(\rv{B})$ \vspace{-1em}
  #+BEGIN_EXPORT latex
  \begin{center}
    \includegraphics<1>[scale=.7]{fig/2sample_comp_2.fig}% ,subfig=2
    \includegraphics<2->[scale=.7]{fig/2sample_comp_4.fig}% ,subfig=4
  \end{center}
  #+END_EXPORT
  \pause
...
- The CLT hypothesis are very weak: it *does not assume any particular
  ...
  /normally distributed/. We have seen it holds true
  quickly\vspace{-.5em}
...
- Fixed Distribution :: (, in particular if the /fixed normal
     distribution/ assumption holds)\vspace{-1.5em}
...
philosopher *William of Occam*:
  ...
  current model
  #+BEGIN_QUOTE
    A model should be as simple as possible. But no simpler.\hspace{-.4em}
    \begin{flushright}
      -- A. Einstein
    \end{flushright}
  #+END_QUOTE
...
- A and B are consequences of a common cause, but do not cause each
  other\hfill$\frowny$
- There is no connection between A and B; it is a 
  #+LaTeX: coincidence\hfill$\frowny$\vspace{-.2em}%
...
- Get rid of it using \eg *quantiles*:
...
** Estimating something else than the mean                        :noexport:
...
- Median :: the /sample median/ is robust to outliers but it is quite
            ...
            want to estimate?
- Minimum and Maximum :: the /sample maximum/ is always too small, hence
     it is biased...
- Variance :: =var= is a unbiased estimator of variance but the "natural
              formula" for standard deviation (e.g., in ~np.std~) is a
              biased estimator.. Unbiasing depends on the distribution
              so just overestimate...
** Statistical Tests                                              :noexport:
*** Tests
