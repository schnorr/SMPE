...
#+LATEX_HEADER: \def\logoInstitutions{\includegraphics[height=1cm]{./images/ufrgs.png}}
...
1. What area/field in which the lack of reproducibility can harm or
   hinder progress in human and technology development the most is?

   - Answer: I think the lack of reproducibility can harm many fields,
     but computer reproducibility (the fact that changing computer
     architecture and software changes results) is one of the most
     important because that can really impact all science fields.

2. Lucas, Why shouldn't we use Git LFS?

   - We could indeed use Git LFS. The problem is that servers usually
     have a size limit for the files. The best solution we have found
     so far is the one provided by Zenodo, which does not have any
     limit size. The advantage of Zenodo is that it provides you with
     a trackable DOI, which is something unavailable in GIT-oriented
     servers (hash commits are usually not tracked - and you can
     rewrite the commit history - so a hash might disappear in the
     future).

3. Some experiments are run over private datasets, or the algorithms
   used are proprietary, at the end, is this kind of research useful
   for the community?

   - If the datasets and software is proprietary, it represents poor
     value for the general science in general because there is no
     transparency, with without transparency, consensus becomes
     impossible.

4. Can we think about having stardardized protocols in the next future
   in computer science?

   - That will be something to discuss indeed. The issue is that
     computer science has so many possible configurations (think about
     the amount of configurations possible for a deep learning
     training session, for instance) that turns out standard protocols
     very hard to establish (it may fix too much the amount of studies
     you could run). But perhaps having a basic set of
     recommendations, such as the ones I've listed during the end of
     the talk will be sufficient.

5. Everybody knows how important reproducibility is, why it should be
   encouraged like it is something new in the social aspect of a
   research?

   - Indeed, I agree with you that the topic of reproducibility
     shouldn't be sell as something new, but something that is
     inherent attached to science in general. The issue is that
     nowadays simply too much research (articles, reports) does not
     use reproducible methods, even if there are enough
     solutions/tools. So, we are basically encouraging people that
     conduct research to actually put into practice these tools that
     enable reproducible research simply because they are available.
...
%  \includegraphics<+>[width=.75\linewidth]{images/iceberg_publication-4.png}%
  ...
  \uncover<.>{\small\bf Reproducible Research = Bridging the Gap by working Transparently}
\end{center}
#+END_EXPORT
...
#+RESULTS:
** Tool 1bis: Lab. Notebooks, Computational Documents              :B_frame:
    :PROPERTIES:
    :BEAMER_env: frame
    :BEAMER_OPT: plain
    :END:
...
#+latex: \vspace{-1cm}\hfill\hbox{\mylogo\hspace{-1cm}}
...
[[https://github.com/schnorr/SMPE/raw/master/lectures/talk_20_11_17_WPerformance.pdf][https://github.com/schnorr/SMPE/raw/master/lectures/talk_23_11_23_ICI2ST_Equador.pdf]]

Thanks to
- [[http://polaris.imag.fr/arnaud.legrand/][Arnaud Legrand]] for sharing slides and knowledge about this topic
- My [[https://www.inf.ufrgs.br/~schnorr/students.html][students]] that have undertaken many of these methods
